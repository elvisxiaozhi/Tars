...
- [x] **1.2** HTTP 客户端封装 (Boost.Beast)
- [x] **1.3** WebSocket 客户端封装
- [x] **1.4** JSON 解析工具
- [x] **1.5** 配置加载 (JSON) + 日志初始化

### Phase 2 — 数据采集与套利检测

- [ ] **2.1** MarketFeed — REST 全量拉取（市场列表 + 订单簿 → 终端打印实时报价）
- [ ] **2.2** MarketFeed — WS 实时更新（增量更新本地订单簿缓存）
- [ ] **2.3** 套利扫描初版（遍历缓存，检测互补套利 Yes+No ask < 1.0，终端打印信号）
