...
### S5. legacy_cheap 不是“低于 30c 就买”，必须加盘口质量过滤
- **背景**：2026-05-04 夜间 dry run，多币种 `legacy_cheap` 41 条 exit record 合计 `-$5.32`。复盘显示亏损集中在 `<10c`、`spread >= 3c`、`minutes_remaining <= 35` 和同小时连续 `stop_price`；止盈记录本身合计为正。
- **决策**：保留 `legacy_cheap` 核心，但入场必须满足 `minutes_remaining > 35`、`0.10 <= entry_price <= 0.30`、`ask - bid <= 0.03`。同币种 stop 后本小时暂停；本小时全局 `stop_price >= 3` 后暂停所有新开仓。
- **为什么不直接换策略**：实验策略当晚只有 4 笔，全部 `stop_price`，样本小且表现更差；直接切换没有数据依据。
- **后果**：交易频率会明显下降，但过滤掉最伤的垃圾盘口。用同一晚日志离线估算：41 条原始记录 `-$5.32`，新过滤保留的 24 条为 `+$2.36`，被过滤的 17 条为 `-$7.68`。这不是正式回测，只作为方向性证据。

### S6. 退役主策略的 QUIET_REVERSION（cheap_rebound），主策略只剩 momentum
- **背景**：2026-05-26 日志复盘。主策略 dry_run 全周期 +$0.04 几乎打平，拆 regime 后利润全来自 `momentum`（+$0.55，86% 胜率/22 仓），`QUIET_REVERSION`（cheap_rebound）是净亏（−$0.51，24% 胜率/21 仓），且最近几天的成交几乎全是亏损的 QR。
- **决策**：退役 QUIET_REVERSION。`evaluate_entry` 仍调 `evaluate_cheap_rebound`（保留 `main_candidates.jsonl` 候选/回放数据），但新增开关 `strategy.quiet_reversion_enabled`（默认 `false`）关闭时把本应成交的 QR 信号改判 `quiet_reversion_retired`，绝不开主仓。代码休眠保留作回放。
- **为什么是退役而非调参**：它与 2026-05-23 退役的 `eth_late_cheap_v1` **同族同标签**（都打 `QUIET_REVERSION`、都"抄便宜侧赌反弹"，只是早窗/晚窗之分），负 edge 量级也一致（−$0.51 vs −$0.475）。晚窗那只已因"无可改杠杆"退役，早窗这只是同一失败模式的换皮，遵循 `strategy-experiments.md` 的"不要重蹈的模式"判据。
- **置信度（诚实标注）**："QR 当前不是正贡献"高把握（净负、24% 胜率、是全部回撤来源）；"统计上确证负 edge"只有中等把握（21 仓 / 10 天小样本，−$0.51 高度依赖 2 个好日子）。退役依据是"找不到 edge + 同门已退役 + 高方差无回报"，不是"证明它必亏"。
- **后果（重要联动）**：主策略现在只在 `abs(dev) ≥ 0.20%` 入场，成交频率显著下降——**低波动日可能整天 0 单**。这正是 2026-05-25 "跑一天 0 成交" 现象的根因放大版：QR 本是平静日唯一还在开仓的东西（且在亏）。这是有意取舍：用频率换掉负 edge。若要恢复频率，方向是改 momentum 入场（0.64-0.67 区间过窄）或填 0.12-0.20 死区，而非复活 QR。
- **回放/恢复**：`config strategy.quiet_reversion_enabled=true` 可恢复（仅用于复测，勿用于实盘）。证据见 [`steps/step-retire-quiet-reversion.md`](./steps/step-retire-quiet-reversion.md)。

### S7. momentum_follow 入场区间从 [0.64, 0.67] 扩到 [0.64, 0.84]
- **背景**：QR 退役后主策略只剩 momentum，但近 30h 0 次入场——934 个 BTC/ETH 趋势带候选全被 `momentum_entry_range` 拒，84% 真实 favored ask 已 ≥ 0.85。原 3¢ 窄缝只捕捉行情刚启动的几秒。
- **决策**：上沿 0.67 → 0.84。回测 71 笔 / 6 天 / 90% 胜率 / 真实 TP 模型 EV +$0.084/笔；BTC-only 30 笔 / 93% 胜率 / +$0.096/笔。模型 1.5x 历史校准折扣后仍是当前 trend +$0.055/天 的 ~10 倍。其余 momentum 规则、TP/stop/trailing 全不变。
- **为什么是 0.84 不是更高**：TP `tp0=min(0.88, E+0.08)` 在 E≥0.85 时 cap 触发使胜的赔付 < 输的赔付（E=0.88 +$0.011 / E=0.90 −$0.011 "胜也亏"）。**0.85 是数学终点，0.84 是安全边距。任何往 0.85+ 扩前必须先重做 TP。**
- **稳健性**：5 项独立检查全过——(A) 真实 TP/stop 模型（替换 +8c/-7c 简化）；(B) 94 个不清晰市场按 50% 胜率灌回仍 EV +$0.075；(C) sep_threshold 0.20-0.50 EV 稳定 +$0.083-0.093；(D) 7 天每天净正，05-21 那天（当前 trend 最大亏损日）放宽后 76% 胜率仍 +$1.29；(E) BTC-only 比 BTC+ETH 还干净。
- **置信度（诚实标注）**：6 天 71 笔回测样本"是正 EV"高把握（~95%）；"未来 30 天延续"中等（~60%，市场会变）；"滑点不会推翻结论"中-高（~75%，限价 maker @ 深价 ask-1c 可能不易成）。
- **回滚阈值**：上线 48h 后若实际胜率 < 75% 或 EV/笔 < +$0.02，立即回滚到 0.67。证据见 [`steps/step-widen-momentum-band.md`](./steps/step-widen-momentum-band.md)。
